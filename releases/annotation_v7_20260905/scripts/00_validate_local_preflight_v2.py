#!/usr/bin/env python3
"""Fail closed unless V7 inputs and engines are local and internally valid."""
...
def main() -> None:
    ...
    for path in sorted((RUN_ROOT / "scripts").glob("*")):
        if path.suffix not in {".py", ".sh"} or path.name.startswith("00_validate_local_preflight"):
            continue
