...
- 将 V7 冻结为当前细胞身份与 CNV 恶性项目参考：4,676,787 cells、1,322 samples、
  54 GSE、43 cancers。
- 将 V7 方法、配置、可审查代码、顶层统计表和重型对象哈希整合为轻量 release。
- 整合 2026-09-06 通讯证据链的报告、汇总表与 Prism/PDF/PNG 成图。
- 明确版本边界：通讯证据链复用 V5 CoVarNet/LIANA，尚未在 V7 上重跑。
- 延续已有 Pancancer_log Git 历史与 private remote；未建立父目录 TB 级 monorepo。
...
- 复核三项 extension：免疫和下载证据未变化；CM 严格完成仍为 580/721。`USER002` group-quota 余量已从约 2.50 GiB 增至约 101.588 GiB，但仍低于 129–157 GiB 需求外推和 200 GiB 重启门槛，因此保持资源阻断且未提交新 recovery。
